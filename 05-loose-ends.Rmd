...
psp_lmm_all <- mixed(
    ...
    REML = FALSE,
    check_contrasts = TRUE,
    all_fit = TRUE, # this is new!
    cl = cl
)

stopCluster(cl)

# check the best optimizer
attr(psp_lmm_all, "all_fit_selected")
attr(psp_lmm_all, "all_fit_logLik")
```

And here we can check for convergence:

```{r all_fit_check}
psp_lmm_all_ok <- psp_lmm_all[sapply(psp_lmm_all, is, "merMod")]
